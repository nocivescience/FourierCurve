...
class FourierScene(Scene):
    ...
    def get_rotating_vectors(self,freqs=None, coeffs=None):
        ...
        for freq, coeff, i in zip(freqs, coeffs, it.count(1)):
            ...
            if random<.5:
                ...
                spin=1
            vector=self.get_rotating_vector(coeff/i*1.2,freq,center_func,spin=spin)
            vectors.add(vector)
            last_vector=vector
        return vectors
    ...
    def add_path(self, mob):
        path = TracedPath(mob.get_end)
        return path